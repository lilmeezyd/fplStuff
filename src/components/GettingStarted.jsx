    ...
    <Modal
      ...
      >
        ...
        <Modal.Body className="p-3">
          ...
          <div  key={idx+1} className="achieve-record">
          <div className="gw py-1">Gameweek {pick?.event}</div>
          {pick?.players?.map(player => 
          <div  key={player?.element}>
          <div className="gw-data py-2">
            <div style={{fontWeight: 500}}>{player.web_name}&nbsp;{player.was_home ? `(H)`:`(A)`}&nbsp;V</div>
              <div>{player.opponent_team}</div>
              <div>{player.team_h_score}:{player.team_a_score}</div>
              <div>{player?.total_points}&nbsp;points</div>
            </div>
            <div className="date">{player?.kickoff_time?.toDateString()}</div>
            </div>
          )}
          </div>
